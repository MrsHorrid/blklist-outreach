  ...
  return (
    ...
    >
      ...
      <div className="rounded-2xl bg-white/[0.02] border border-white/[0.06] backdrop-blur-xl p-7 shadow-[0_1px_0_rgba(255,255,255,0.05)_inset]">
        <h1 className="text-white text-[22px] font-semibold tracking-tight mb-1.5">Create account</h1>
        <p className="text-white/40 text-[13px] mb-6">Start discovering leads with AI</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          <Input label="Name" type="text" value={form.name} onChange={v => setForm(f => ({ ...f, name: v }))} placeholder="Alex Johnson" required />
          <Input label="Email" type="email" value={form.email} onChange={v => setForm(f => ({ ...f, email: v }))} placeholder="you@company.com" required />
          <Input label="Password" type="password" value={form.password} onChange={v => setForm(f => ({ ...f, password: v }))} placeholder="Min. 8 characters" required minLength={8} />
          ...
          >
            {loading ? <DotSpinner color="#0a0a0c" /> : 'Create account'}
          </button>
        </form>
      </div>
      ...
      <p className="text-center text-white/40 text-[13px] mt-7">
        ...
        <Link href="/login" className="text-white hover:text-white/85 font-medium transition-colors">
          Sign in
        </Link>
      </p>
    </motion.div>
...
function Input({ label, type, value, onChange, placeholder, required, minLength }: {
